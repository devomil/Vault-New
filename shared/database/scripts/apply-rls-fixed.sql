-- Apply Row-Level Security (RLS) Policies (Fixed)
...
-- Create RLS policies for each table (using correct camelCase column names)
...
CREATE POLICY user_tenant_isolation_policy ON vault.users
    FOR ALL USING ("tenantId" = vault.get_tenant_id());
...
CREATE POLICY product_variant_tenant_isolation_policy ON vault.product_variants
    FOR ALL USING (
        "productId" IN (
            ...
            WHERE "tenantId" = vault.get_tenant_id()
        )
    );
